...
def benchmark_mm(m, n, k, label, b=None, num_iterations=200):
    B = torch.randn((k, n)).half().to("cuda:2")
    if b is None:
        A = torch.randn((m, n)).half().to("cuda:2")
        C = torch.empty((m, k)).half().to("cuda:2")
        b = 1
    else:
        A = torch.randn((b, m, n)).half().to("cuda:2")
        C = torch.empty((b, m, k)).half().to("cuda:2")
...
def benchmark_bmm(b, m, n, k, label, num_iterations=200):
    A = torch.randn((b, m, n)).half().to("cuda:2")
    B = torch.randn((b, n, k)).half().to("cuda:2")
    C = torch.empty((b, m, k)).half().to("cuda:2")
    num_warmup_iterations = 50
...
def benchmark_dropout(A_dim, label, num_iterations=100):
    A = torch.randn(A_dim).half().to("cuda:2")
    dropout = torch.nn.Dropout(0.5).to("cuda:2")
    num_warmup_iterations = 50
...
def benchmark_softmax(scores_shape, seq_length, label, num_iterations=100):
    scores = torch.randn(scores_shape).half().to("cuda:2")
    attention_mask = torch.tril(torch.ones(
        (1, seq_length, seq_length), device="cuda:2")).view(
...
def benchmark_fused_gelu(A_dim, b_dim, label, num_iterations=100):
    A = torch.randn(A_dim).half().to("cuda:2")
    b = torch.randn(b_dim).half().to("cuda:2")
    num_warmup_iterations = 50
...
def benchmark_layer_norm(A_dim, normalized_shape, label, num_iterations=100):
    A = torch.randn(A_dim).half().to("cuda:2")
    layer_norm = LayerNorm(normalized_shape).half().to("cuda:2")
    num_warmup_iterations = 50
    for i in range(num_warmup_iterations + num_iterations):
        if i == num_warmup_iterations:
            start_time = time.time()
...
def benchmark_add_bias_dropout(shape, label, num_iterations=100):
    A = torch.randn(shape).half().to("cuda:2")
    bias = torch.randn(shape).half().to("cuda:2")
    residue = torch.randn(shape).half().to("cuda:2")
    num_warmup_iterations = 50
...
def benchmark_transformer(configuration, seq_length, global_batch_size, num_iterations=100):
    ...
    attention_layer = ParallelSelfAttention(args,attention_mask_func=attention_mask_func, init_method=init_method,output_layer_init_method=init_method, layer_number=0).half().to("cuda:2")
    mlp_layer = ParallelMLP(args,init_method=init_method,output_layer_init_method=init_method).half().to("cuda:2")
    transformer_layer = ParallelTransformerLayer(args,attention_mask_func=attention_mask_func,init_method=init_method,output_layer_init_method=init_method,layer_number=0).half().to("cuda:2")
    inp = torch.randn((args.seq_length, args.batch_size, args.hidden_size)).half().to("cuda:2")
    attention_mask = torch.tril(torch.ones(
        (1, args.seq_length, args.seq_length), device="cuda:2")).view(
...
if __name__ == '__main__':
    ...
    for tensor_mp_size in [1]:
        for num_attention_heads in [32]:# [32,128]: #[32, 64, 96, 128]:
